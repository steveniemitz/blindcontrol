...
import websockets
from gizapi import GizToken, GizApi
from frames import FrameData, FrameEncoder, CommandFrame, FrameType, Header, FrameDecoder, DEFAULT_DECODER, DEFAULT_ENCODER
...
class DeviceDiscovery:
  def __init__(self, api: GizApi, token: GizToken, enc: FrameEncoder=None, dec: FrameDecoder=None, url: str='wss://ussandbox.gizwits.com:8880/ws/app/v1'):
    self._url = url
    self._token = token
    self._api = api
  ...
  async def _list_devices(self, ws, did):
    msg = CommandFrame(
      ...
      frame_type=FrameType.DEVICE_LIST_REQ
    )
  ...
  async def _query(self, devices: List[Device]):
    ...
    async with websockets.connect(self._url, ssl=True) as ws:
      await ws.send(self._login_msg(token))
      login_resp = await ws.recv()
      devices = [Device(d.did, d.channel, d.name, await self._query_device(ws, d)) for d in devices]
      return devices
  ...
  async def _query_device(self, ws, d: Device):
    ...
    inner_para_data = [f for f in decoded.data if f.key == DataKeys.INNER_PARA_DATA.value]
    if inner_para_data:
  ...
  def discover(self):
    devices: List[(str, CommandFrame)] = _LOOP.run_until_complete(self._discover())
    ret = []
    for (did, d) in devices:
      channels = [c.value for c in d.data if c.key == DataKeys.DEVICE_ADDR_CHANNEL.value]
      names = [c.value for c in d.data if c.key == DataKeys.NAME.value]
      positions = [int(c.value[1]) for c in d.data if c.key == DataKeys.DEVICE_CMD_DATA.value]
      ret += [Device(did, channel, name, position) for (channel, name, position) in zip(channels, names, positions)]
    return ret
