...
from frames import CommandFrame, Header, FrameType, FrameData, MotoCmd
from frame_constants import DataKeys
from auth.oauth2 import require_oauth, get_user_for_token
...
class GoogleHome:
  def __init__(self, api: GizApi):
    ...
    self.bp.add_url_rule('/googlehome', '_handle_request', self._handle_request, methods=['POST'])

  @require_oauth()
  def _handle_request(self):
    user = get_user_for_token(current_token.access_token)
    giz_token = GizToken(user.gizToken, user.gizUid, user.gizExpireAt, user.username, user.password)
        ...
        for e in c['execution']:
          ...
          frame = CommandFrame(
            ...
            data=[
              ...
              FrameData(DataKeys.DEVICE_CMD_DATA.value, bytes([1, 100 - pct, 0]))
            ]
          )
