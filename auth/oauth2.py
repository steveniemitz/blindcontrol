import urllib.parse
from flask import Blueprint, request, make_response, redirect, session, render_template
from authlib.flask.oauth2 import AuthorizationServer, ResourceProtector
from authlib.specs.rfc6749 import grants, OAuth2Error
from authlib.specs.rfc6750 import BearerTokenValidator
from werkzeug.security import gen_salt
import time
from .models import (User, OAuth2AuthorizationCode, OAuth2Client)
...
class OAuth:
  def __init__(self, api: GizApi):
    ...
    self.bp.add_url_rule('/oauth/login', 'show_login', self.show_login, methods=['GET'])
    self.bp.add_url_rule('/oauth/authorize', 'authorize', self.authorize, methods=['GET', 'POST'])
    self.bp.add_url_rule('/oauth/token', 'token', self.issue_token, methods=['POST'])
  ...
  def authorize(self):
    def _make_redirect():
      return redirect('/oauth/login?return=%s' % urllib.parse.quote(request.url))
